...
def customer_to_json():
    ...
    cur.execute("SELECT CivilianPersonId, name, phone, address, faceimg, lower "
...
def get_age_span(ages):
    span_count = [0, 0, 0, 0, 0, 0]
    maxa = 0
    l = h = 0
    for a in ages:
        if 10 <= a <= 15:
            span_count[0] += 1
            if span_count[0] > maxa:
                maxa = span_count[0]
                l = 10
                h = 15
        elif 16 <= a <= 20:
            span_count[1] += 1
            if span_count[1] > maxa:
                maxa = span_count[1]
                l = 16
                h = 20
        elif 21 <= a <= 25:
            span_count[2] += 1
            if span_count[2] > maxa:
                maxa = span_count[2]
                l = 21
                h = 25
        elif 26 <= a <= 30:
            span_count[3] += 1
            if span_count[3] > maxa:
                maxa = span_count[3]
                l = 26
                h = 30
        elif 31 <= a <= 35:
            span_count[4] += 1
            if span_count[4] > maxa:
                maxa = span_count[4]
                l = 31
                h = 35
        elif 36 <= a <= 40:
            span_count[5] += 1
            if span_count[5] > maxa:
                maxa = span_count[5]
                l = 36
                h = 40
    return l, h
...
def add_new_civilian(civilian, addperson):
    ...
    with app.app_context():
        ...
        if civilian.gender.gender == 1:
            ...
            cil_gender = 2
        l, h = get_age_span(civilian.predicteda)
        for a in civilian.predicteda:
            if l<=a<=h:
                civilian.lower = a
...
def civilian_out(civ, outtime, outdate):
    with app.app_context():
        ...
        for e in civ.expres:
            ...
            if e == 'neutral':
                neutral_count += 1
                if neutral_count > maxe:
                    maxe = neutral_count

        if sad_count == maxe:
            add_emotion(civ, 'sad')
        if happy_count == maxe:
            add_emotion(civ, 'happy')
        if neutral_count == maxe:
            add_emotion(civ, 'neutral')
        cur.close()
...
def get_civilian_byday(day):
    ...
    cur.execute("Select PersonId, GenderId, expression, lower, timein, datein, "
                ...
                "on PersonId = Expression.CivilianPersonId and datemoment = datein and timein = moment "
                "WHERE datein = %s) as L "
                "LEFT JOIN Customer "
